...
GRSurface* ScreenRecoveryUI::GetCurrentFrame() {
    ...
    } else if(currentIcon == INSTALLING_UPDATE_MOBILE) {
        return icon_installing_mobile;
    } else if(currentIcon == ERASING_MOBILE) {
        return erasing_mobile;
    } else if(currentIcon == ERROR_MOBILE) {
        return icon_error_mobile;
    }
    return error_icon;
}
...
void ScreenRecoveryUI::draw_foreground_locked() {
    ...
    if (currentIcon != NONE) {
        ...
        if (mobile_flag == 1) {
            // frame_y = frame_y + gr_get_height(loopFrames[0])/2; // baseparam is 1080p
            frame_y = frame_y + gr_get_height(loopFrames[0]);
        } else if (baseInfo == 720) {
            frame_y = frame_y + gr_get_height(loopFrames[0])*2/3;
        }
...
*/
        ...
        if (mobile_flag == 0 && baseInfo == 720) {
            progress_y = progress_y*6/5;
        }
        ...
        if (progressBarType == INDETERMINATE) {
            if((mobile_flag == 1)) {
                ...
                static int j = 0;
                static int k = 1;
                int width_block = gr_get_width(block);

                gr_blit(progressBarEmpty, 0, 0, width, height, progress_x, progress_y);
                if (!rtl_locale) {
                    if (length < (width - width_block) && (j == 0)){
                        length = 8 * i;
                        gr_blit(block, 0, 0, width_block, height, progress_x + length, progress_y);
                        i += 1;
                        k = 1;
                    } else {
                        j = 1;
...
void ScreenRecoveryUI::Init() {
    ...
    if(mobile_flag == 1) {
        LoadBitmap("icon_installing_mobile", &icon_installing_mobile);
        LoadBitmap("erasing_mobile", &erasing_mobile);
        LoadBitmap("icon_error_mobile", &icon_error_mobile);
        LoadBitmap("block", &block);
...
#ifdef HI_REMOTE_RECOVERY
    LoadBitmap("icon_menu", &icon_menu);
    LoadBitmap("icon_background0", &selectedBackgroundIcon[0]);
    LoadBitmap("icon_background1", &selectedBackgroundIcon[1]);
    LoadBitmap("icon_background2", &selectedBackgroundIcon[2]);
    LoadBitmap("icon_background3", &selectedBackgroundIcon[3]);
#endif
...
void ScreenRecoveryUI::draw_selected_background()
{
    pagesIdentical = false;
    gr_color(0, 0, 0, 255);
    gr_fill(0, 0, gr_fb_width(), gr_fb_height());
    GRSurface* frame = selectedBackgroundIcon[menu_sel];
    int iconWidth = gr_get_width(frame);
    int iconHeight = gr_get_height(frame);
    int iconX = (gr_fb_width() - iconWidth) / 2;
    int iconY = GetAnimationBaseline();
    gr_blit(frame, 0, 0, iconWidth, iconHeight, iconX, iconY);
    gr_flip();
}
