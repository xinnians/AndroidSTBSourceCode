                  ...
                  std::vector<std::string>& log_buffer, int retry_count)
{
    ...
    printf("----------vmx advanced----------\n");
    const char* binary = "/sbin/updater";
#else
                       ...
                       std::vector<std::string>& log_buffer, int retry_count)
{
    ...
    if (check_local_fastboot_valid()){
        ...
        err = common_verify(&zip, key_area, "bootargs.img");
        if (err != VERIFY_SUCCESS){
            LOGE("Verify bootargs failed!\n");
            mzCloseZipArchive(&zip);
            return INSTALL_CORRUPT;
        }

        err = special_verify(&zip, key_area, "boot.img");
        if (err != VERIFY_SUCCESS){
        ...
        err = special_verify(&zip, key_area, "recovery.img");
        if (err != VERIFY_SUCCESS){
            LOGE("Verify recovery failed!\n");
            mzCloseZipArchive(&zip);
            return INSTALL_CORRUPT;
        }

        err = special_verify(&zip, key_area, "trustedcore.img");
        if (err != VERIFY_SUCCESS){
            LOGE("Verify trustedcore failed!\n");
            mzCloseZipArchive(&zip);
            return INSTALL_CORRUPT;
        }
    }
    ...
    sysReleaseMap(&map);
    return try_update_binary(path, &zip, wipe_cache, log_buffer, retry_count);
