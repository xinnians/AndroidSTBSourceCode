...
static const char *dev = "/dev/block";
...
#define SDCARD_UPDATE "/sdcard/update.zip"
#define INSTALL_PACKAGE_FROM_BACKUP "/backup/update.zip"
unsigned int read_timeout = 200;
int g_thread_should_stop = 0;

int update_UI = 0;
int emergency_recovery = 0;
RecoveryUI* ui = NULL;
...
static bool reset_baseparam = false;
...
get_args(int *argc, char ***argv) {
    ...
    if (fp != NULL) {
        ...
        if (fgets(buf, sizeof(buf), fp)) {
            ...
            printf("VMX branch buf is %s\n",buf);
            if(strstr(buf, "/dev/block/vold")) {
                char *tmp = strstr(buf, "mount_point=\"");
                int result = 0;
                char *mount_point = NULL;
                mount_point = (char *)malloc(MAX_ARGS);
                char *dev_node = NULL;
                dev_node = (char *)malloc(MAX_ARGS);
                char *update_dev = NULL;
                update_dev = (char *)malloc(MAX_ARGS);
                char *buffer = NULL;
                buffer = (char *)malloc(MAX_ARGS);
                if (mount_point == NULL || dev_node == NULL
                        || update_dev == NULL || buffer == NULL) {
                ...
                if (tmp) {
                    char *p;
                    strcpy(buffer, tmp+13);
                    p = strtok(buffer, "\"");
                    strcpy(mount_point,p);
                }
                if(strstr(mount_point,"mnt/media_rw/sd")) {
                   diskFlag = find_upzip_and_update_volume();
                } else if(strstr(mount_point,"mnt/media_rw/mmcblk1")) {
                    dev_node = mount_point + strlen("mnt/media_rw/mmcblk1") + 1;
                    strcpy(update_dev, dev);
                    strcat(update_dev, dev_node);
                    diskFlag = update_sdcard_volume_by_dev_node(update_dev);
                }
            FREEMEM:
                if(mount_point != NULL) {
                    free(mount_point);
            ...
            if (tmp) {
                ...
                if (tmp != NULL) {
                   ...
                   if(strncmp(p, "ext", 3) == 0 || strncmp(p,"vfat",4) ==0 || strncmp(p,"ntfs",4) ==0) {
                       ...
                       diskFlag = update_sdcard_volume_by_uuid((const char*)buf_uuid, buf_fs);
                   } else {
                       diskFlag = update_sdcard_volume_by_uuid((const char*)buf_uuid, NULL);
                   }
                } else {
                   diskFlag = update_sdcard_volume_by_uuid((const char*)buf_uuid, NULL);
                }
                ...
                if (path) {
                    diskFlag = update_sdcard_volume_by_path((const char*)(path + 5), "ubifs");
                }
            }
...
get_menu_selection(const char* const * headers, const char* const * items,
    ...
    while (chosen_item < 0) {
        ...
        HI_UNF_IR_GetValueWithProtocol(&status, &key, name, sizeof(name), read_timeout);
        int action = device->HandleMenuKey(key, val);
...
*/
    // The PACKAGE_FILE must be update.zip
    const char *PACKAGE_FILE = UpdatePackageName;
...
static int apply_from_sdcard(Device* device, bool* wipe_cache) {
    ...
    if (find_upzip_and_update_volume() == -1) {
        ...
        if (erase_volume("/data")) status = INSTALL_SUCCESS;
        if (erase_volume("/cache")) status = INSTALL_SUCCESS;
    ...
    result = install_package(SDCARD_UPDATE, wipe_cache,
...
prompt_and_wait(Device* device, int status) {
    for (;;) {
        ...
        {
            ui->StartUI();
            int chosen_item = get_menu_selection(nullptr, nullptr, 0, 0, device);
            chosen_action = device->InvokeMenuItem(chosen_item);
            printf("update_UI ");
        } else{
            chosen_action = Device::APPLY_SDCARD;
            printf("force update !!!!!\n");
        }
#else
        ...
        switch (chosen_action) {
            ...
            case Device::WIPE_CACHE:
                ...
                if (!ui->IsTextVisible()) return Device::NO_ACTION;
                break;
            case Device::UPDATE_VOLUME:
                ui->SetBackground(RecoveryUI::INSTALLING_UPDATE);
                ret_status = install_package(INSTALL_PACKAGE_FROM_BACKUP, &should_wipe_cache, TEMPORARY_INSTALL_FILE, true, 0);
                if (ret_status == INSTALL_SUCCESS && should_wipe_cache) {
                    if (erase_volume("/cache")) {
                        LOGE("Cache wipe (requested by package) failed.");
                    }
                }
                if (ret_status != INSTALL_SUCCESS) {
            ...
            case Device::APPLY_SDCARD:
                {
                    ...
                    if (adb) {
                        ...
                        printf("--> call apply_from_sdcard, return = %d \n", status);
                    }
...
void reset_partname(char *partname){
    char buf_img[64]={0};
    if (ensure_path_mounted("/system") != 0) {
        ui->Print("reset_partname:Can't mount system\n");
        return ;
    }
    Volume *vblock= volume_for_path(partname);
    sprintf(buf_img,"/system/etc/%s.img",partname+1);
    copyFile(buf_img,vblock->blk_device);
...
int main(int argc, char **argv) {
...
#ifdef HI_REMOTE_RECOVERY
    int ret = HI_UNF_IR_Init();
    if (ret) {
        printf("Fail to open ir dev!\n");
        exit(-1);
    }
    else{
        printf("Success to open ir dev!\n");
    }
    int rt = 0;
    if(send_intent){
    ...
    if (update_package) {
        ...
        // "/cache/foo".
        if(strstr(update_package, INSTALL_PACKAGE_FROM_BACKUP)) {
    ...
    Device::BuiltinAction temp = Device::NO_ACTION;
    if (update_package != NULL && diskFlag == 0) {
        ...
        temp = prompt_and_wait(device, status);
    }
	...
	if(reset_baseparam) {
		ui->Print("reset baseparam...\n");
		reset_partname("/baseparam");
	}
