...
int
GetMtdInfo(int fd, struct mtd_info_user *info)
{
    int rel = -1;
    if ((rel = ioctl(fd, MEMGETINFO, info)) != 0) {
        printf("ioctl MEMGETINFO fail, fd=0x%x.\n", fd);
    }
    return rel;
}
...
static inline int page_empty(unsigned char *buf, int pagesize)
{
    int * nn = (int *)buf;
    pagesize = (pagesize >> 2);
    while (pagesize--) {
...
static int write_ubi_block(MtdWriteContext *ctx, const char *data, int length)
{
    const MtdPartition *partition = ctx->partition;
    int fd = ctx->fd;
    int offset_inblock = 0;
    int ret = 0;
    int page_count = 0 ;
    ssize_t size = 0;
    struct mtd_epage_buf page_buf;
    struct mtd_info_user mtd_info;
    GetMtdInfo(ctx->fd, &mtd_info);
    int oob_count = (mtd_info.erasesize/mtd_info.writesize);
    off_t pos = lseek(fd, 0, SEEK_CUR);
    if (pos == (off_t) -1) return 1;
    if((unsigned int )length >= mtd_info.erasesize)
        size = partition->erase_size;
    else
        size = length;
    unsigned char* oobbuffer = malloc(mtd_info.oobsize);
    memset(oobbuffer,0xff,mtd_info.oobsize);
    while (pos + size <= (int) partition->size) {
        loff_t bpos = pos;
        ...
        for (retry = 0; retry < 2; ++retry) {
            ...
            for(offset_inblock = 0; offset_inblock < size; offset_inblock += mtd_info.writesize) {
                page_buf.data_ptr = (unsigned char *)data + offset_inblock;
                page_buf.data_len = mtd_info.writesize;
                if (page_empty(page_buf.data_ptr, page_buf.data_len)) {
                    page_buf.oob_len = mtd_info.oobsize;
                    page_buf.oob_ptr = oobbuffer;
                    page_buf.start = pos + offset_inblock;
                    ret = ioctl(fd, MEMEWRITEPAGE, &page_buf);
                    if ( ret!= 0) {
                        free(oobbuffer);
                        return -1;
                    }
            ...
            char verify[partition->erase_size];
            if (lseek(fd, pos, SEEK_SET) != pos ||
                    read(fd, verify, (size_t)partition->erase_size) != (ssize_t)partition->erase_size) {
...
ssize_t mtd_write_ubi_data(MtdWriteContext *ctx, const char *data, size_t len)
{
    size_t wrote = 0;
    if (write_ubi_block(ctx, data + wrote, len - wrote)) return -1;
    wrote = len;
    return wrote;
}
