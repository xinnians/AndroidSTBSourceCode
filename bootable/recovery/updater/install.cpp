...
extern char *IsUpdatePackage;
extern char* UpdateName;
...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(fs_type,"raw") == 0 && strcmp(partition_type,"EMMC") == 0) {
        ...
        if (strstr(location,"/dev/block/platform/soc/f9830000.himciv200.MMC/by-name") != NULL) {
            sprintf(partition_path,"%s",location);
        } else {
            sprintf(partition_path,"/dev/block/platform/soc/f9830000.himciv200.MMC/by-name/%s",location);
        }
        dir = opendir("/dev/block/platform/soc/f9830000.himciv200.MMC/by-name/");
        bool_bit = 0;
        if (dir) {
            while((dirent = readdir(dir))) {
                memset(dir_buf,0,sizeof(dir_buf));
                strcpy(dir_buf,"/dev/block/platform/soc/f9830000.himciv200.MMC/by-name/");
...
Value* PackageExtractFileFn(const char* name, State* state,
    ...
    if (argc == 2) {
...

        {
        ...
        #if (defined Hi3798MV100) || (defined Hi3796MV100)
                lseek(fd,512,SEEK_SET);
        #endif
            if (write(fd,v->data,v->size) < 0) {
            ...
            if (strstr(dest_path,"kernel") || strstr(zip_path,"bootargs")
                         || strstr(zip_path,"recovery")
                         || strstr(zip_path,"shatable")) {
                printf(" VMX_ADVCANED update %s\n",zip_path);
                return vmx_verify(zip_path, dest_path, za, state);
            }
        #endif
...
Value* IsUserVersion(const char* name, State* state, int argc, Expr* argv[]) {
    char *result = NULL;
#ifdef USER_VERSION
    result = "ok";
#else
...
Value* IsExistUpdatePackage(const char* name, State* state, int argc, Expr* argv[]) {
    char* path;
    bool success = false;
    if (ReadArgs(state, argv, 1, &path) < 0) {
        return NULL;
    }
    if(IsUpdatePackage !=NULL) {
        if (!strncmp(IsUpdatePackage, path, strlen(path))) {
            success = true;
        }
    }
    printf("IsUpdatePackage success=%d,path=%s : %s\n",success,path,IsUpdatePackage);
    return StringValue(strdup(success ? "ok" : ""));
}
/*
int flashled_pid = -1;
...
Value* CopyFileFn(const char* name, State* state,
        int argc, Expr* argv[]) {
    if (argc != 2) {
    ...
    char* targetFile;
    char* sourceFile;
    char buf_update[128] = {0};
    bool success = false;
    if (ReadArgs(state, argv, 2, &sourceFile, &targetFile) < 0) return NULL;
    if(!strcmp(sourceFile,UpdateName)) {
        sprintf(buf_update,"%s/%s",IsUpdatePackage,sourceFile);
    }else
        sprintf(buf_update,"%s",sourceFile);
    if (copyFile(buf_update,targetFile) == 0) {
        success = true;
    }
    free(sourceFile);
    free(targetFile);
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("isexistupdatepackage",IsExistUpdatePackage);
    RegisterFunction("isuserversion",IsUserVersion);
//    RegisterFunction("open_led", OpenLed);
//    RegisterFunction("close_led", CloseLed);
    RegisterFunction("copy_file", CopyFileFn);
}
