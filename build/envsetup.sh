...
function setpaths()
{
    ...
    # add hisilicon toolchains
    HISI_TOOLCHAINS=$T/device/hisilicon/bigfish/sdk/tools/linux/toolchains/arm-histbv310-linux/bin:$T/device/hisilicon/bigfish/sdk/tools/linux/toolchains/aarch64-histbv100-linux/bin:$T/device/hisilicon/bigfish/sdk/tools/linux/utils/bin

    export PATH=$HISI_TOOLCHAINS:$ANDROID_BUILD_PATHS$PATH
    export PYTHONPATH=$T/development/python-packages:$PYTHONPATH

    # add libconscrypt_openjdk_jni.so
    CONSCRYPTOPENJDKLIB=$T/device/hisilicon/bigfish/prebuilts/ConsCryptOpenJDKLib
    export LD_LIBRARY_PATH=$CONSCRYPTOPENJDKLIB:$LD_LIBRARY_PATH
    ...
    if [ -n "$JAVA_HOME" ]; then
        ...
        # add prebuilt java8
        export ANDROID_JAVA_HOME=$T/device/hisilicon/bigfish/prebuilts/toolchains/java-8-openjdk-amd64
        export ANDROID_JAVA_TOOLCHAIN=$ANDROID_JAVA_HOME/bin
...
{
    sh device/hisilicon/bigfish/build/kernel_menuconfig.sh
}
...
function cbigfish()
{
    T=$(gettop)
    if [ "$T" ]; then
        \cd $(gettop)/device/hisilicon/bigfish
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
function chidolphin()
{
    T=$(gettop)
    if [ "$T" ]; then
        \cd $(gettop)/device/hisilicon/bigfish/hidolphin
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
    fi
}
...
if [ $USE_CCACHE ];then
    echo "USE_CCACHE=$USE_CCACHE, disable ccache!"
    unset USE_CCACHE
fi
if [ $CCACHE_DIR ];then
    echo "CCACHE_DIR=$CCACHE_DIR, disable ccache!"
    unset CCACHE_DIR
fi
