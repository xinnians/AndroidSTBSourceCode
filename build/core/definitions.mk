...
# add by hisi for jack release
define multi-prebuilt-jack-libs
    $(foreach t, $(1), \
    $(eval loop_lib = $(subst :,$(space),$(t))) \
    $(eval target_path = $(firstword $(loop_lib))) \
    $(eval origin_path = $(word 2, $(loop_lib))) \
    $(if $(strip $(origin_path)),$(space), $(eval origin_path = $(target_path))) \
    $(eval JACK_INTERMEDIATE_DIR := $(call intermediates-dir-for, JAVA_LIBRARIES, \
    $(target_path), $(LOCAL_IS_HOST_MODULE), COMMON)) \
    $(eval $(shell mkdir -p $(ANDROID_BUILD_TOP)/$(JACK_INTERMEDIATE_DIR))) \
    $(eval $(shell cp -f $(LOCAL_PATH)/$(origin_path).jack $(ANDROID_BUILD_TOP)/$(JACK_INTERMEDIATE_DIR)/classes.jack)) \
)
endef
# end add
